...
│                                                              │
│  ① RSS フィード取得（FX専門・世界情勢・日本情勢）            │
│      → LLM でセンチメント分析                                │
│      → nomic-embed-text でベクトル化 → ChromaDB に蓄積       │
│  ② yfinance で OHLCV 取得 → SQLite にキャッシュ（差分取得）  │
│  ③ テクニカル指標計算                                        │
│      （SMA/EMA/RSI/MACD/BB/ATR/ADX/一目均衡表）              │
│  ④ LLM でテクニカル分析                                      │
...
│  Phase 3: テクニカルスナップショットを時間加重集約（直近8h）  │
│           （スナップショット未蓄積時は LLM 即時分析に退避）   │
│  Phase 4: RAG からニュースセンチメントを集約                  │
│           → シグナル統合（テクニカル60% + ニュース40%）       │
│           → BUY/SELL/HOLD 判定（HOLD時も方向予測を表示）      │
│  Phase 5: ペーパー注文執行・レポート出力                      │
└──────────────────────────────────────────────────────────────┘
```
