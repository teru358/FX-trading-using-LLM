...
複数のLLMモデルを使い分けて同一ペアの分析結果を比較検証できます。
...
1. **モデル探索** — Ollama（ローカル）・Gemini・OpenAI・Claude の利用可能モデルを自動検出
2. **モデル選択** — 以下3フェーズのモデルをそれぞれ対話的に選択
   - **[A] ファンダメンタル分析** — RSSを1回取得し、選択モデルでFX/Global/Japanの3カテゴリを分析
   - **[B] テクニカル分析** — OHLCVと指標を選択モデルで分析
   - **[C] 総合分析** — [A][B]の結果を別モデルで統合し、最終的な発注判断を生成
3. **結果表示** — 各フェーズの分析結果（方向・スコア・SL/TP・根拠）をターミナルに表示
4. **TXT保存**（任意） — `data/compare/YYYYMMDD_HHMMSS_<pair>.txt` に保存
...
| SL / TP | ストップロス・利確価格 |
| R/R | リスクリワード比 |
| 一致度 | agree / partial / disagree（[A][B]の方向性一致度） |
| 一致点 / リスク | 両分析の合意点・乖離ポイント（日本語） |
| 理由 | 総合判断根拠（日本語） |

> **注意**: 1回の `compare` 実行内では全モデルが同一RSSデータを共有するため公平な比較が可能です。
> ただし複数回実行する場合、実行間でRSS記事が変わることがあります。
