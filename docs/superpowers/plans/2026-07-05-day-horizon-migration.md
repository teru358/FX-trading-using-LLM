...
6. **watch 経路の 1h 固定 (Task 13 follow-up):** spec §9-5 の NG を修正済み — watch 銘柄は `_ohlcv_interval_for` で 1h 固定、MTF は base より細かい TF を skip。**正確な残余 (最終レビューで判明):** 収集・staleness は従来どおりだが、day 設定 (mtf short=15m) 下では watch の short TF が skip され、`_collect_one` の fallback により watch は **1h 単一 TF 分析に縮退**する (従来は long+medium+short composite)。crash はせず watch は発注を gate しないため実害は限定的。対処選択肢: (a) このまま受容 (watch=マクロ文脈、1h は従来 short と同粒度) / (b) watch 専用 MTF timeframe セット。**判断はユーザー — codex 再レビュー時の adjudicate 項目。**
7. **codex 1巡目 (2026-07-05, gpt-5.5/xhigh, d8a209d..f415fb5) 反映済み — 6 コミット:** High#1 ATR の 1h 前提 (`fcb1fb5`) / High#2 hindsight の interval 欠落 (`1ecef5f`) / Med#1 yfinance・TD の interval 未伝搬 (`727dfd0`) / Med#2 context stale 30分固定→config 導出 (`ae84e7f`) / Low metrics horizon フィルタ (`bfa7913`) / 検証レビューで発見した同族残存 3 箇所の掃討 (vol_regime resample・audit/econ の load_ohlcv, `80bfd1d`)。全件 5 件 RESOLVED 判定 + swing 既定値で挙動不変を検証済み。suite 1338 passed。教訓: 生産者側 (fetch/cache) の interval 化だけでなく**消費者側の「1h が基底」前提の全数掃討**が必要だった — `load_ohlcv(`/`resample` の grep 棚卸しをチェックリスト化すべき。
