...
- **実行する「分」は news と衝突しない値を動的に選ぶ (外部レビュー High 対応)**。
  当初は `technical_times` (毎時 `:00`) を借用していたが、news は
  `offset_minutes` (既定 0) + `interval_minutes` により `:00/:15/:30/:45` を占め、
  `PriorityJobSlot` は競合時に**待機せず即スキップ**するため `:00` では
  reflection か news の**どちらかが必ず欠落**していた。
  → news の実時刻集合から未使用の分を選ぶ (既定設定では `:07`)。
  technical / exit_check / price_monitor の時刻も回避対象に含める。
  病的設定 (`interval_minutes=1` 等) で全分が埋まる場合は、
  **衝突を受け入れてでも登録を残す** (欠落 > 衝突)。
- **1 回の実行に経過時間の上限**を設ける (600 秒)。件数上限 (10) だけでは
  ローカル LLM が遅いときに後続ジョブを圧迫するため。上限判定は
  「次の 1 件に着手する**前**」にのみ置き、着手済みの件は必ず done/retry まで
  到達させる (未着手の件には DB 行を作らず、次回へ持ち越す)。
- 本質的な解は **scheduled job 用の優先度付きキュー** (`news > reflection` で
  待機実行) だが、これは cycle-retirement のスコープ外として将来課題に置く。
