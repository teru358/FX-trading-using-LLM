...
halt 判定は BridgeHealthGate に一元化されており、PriceProvider は持たない。
MT5 fetcher の Mt5UnreachableError は単に fallback トリガとして扱う。
"""
...
class PriceProvider:
    ...
    halt 判定は BridgeHealthGate に集約。PriceProvider は単純な fallback chain のみ担う。
    """
    ...
    def get_current_price(self, symbol: str, is_monitor: bool = False) -> CurrentPrice:
        """現在価格を取得する（同期）。MT5 → TD → yfinance の順。"""
        if (
    ...
    def get_ohlcv(
        ...
        """OHLCV を取得する（同期）。trade ペア: MT5 → TD → yfinance。"""
        prev = self._active_provider.get(symbol, "mt5")
