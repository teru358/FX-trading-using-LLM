...
def _setup(tmp_state_store) -> tuple[PositionManager, Order]:
...
def test_adjust_reduces_size(tmp_state_store):
    pm, order = _setup(tmp_state_store)
...
def test_adjust_to_zero_raises(tmp_state_store):
    pm, order = _setup(tmp_state_store)
    with pytest.raises(ValueError, match="<= 0"):
        pm.adjust_position_size(order.order_id, 0)


def test_adjust_negative_raises(tmp_state_store):
    pm, order = _setup(tmp_state_store)
    with pytest.raises(ValueError, match="<= 0"):
        pm.adjust_position_size(order.order_id, -1)


def test_adjust_larger_raises(tmp_state_store):
    pm, order = _setup(tmp_state_store)
    with pytest.raises(ValueError, match="larger"):
        pm.adjust_position_size(order.order_id, 1500)


def test_adjust_unknown_returns_none(tmp_state_store):
    pm, _order = _setup(tmp_state_store)
    result = pm.adjust_position_size("mt5:nonexistent", 500)
    assert result is None


def test_adjust_persists_to_state_store(tmp_state_store):
    pm, order = _setup(tmp_state_store)
...
def test_balance_unchanged_after_adjust(tmp_state_store):
    pm, order = _setup(tmp_state_store)
